...
def evaluate4(loader, model, epoch, config, test=False):
    start = time.time()
    print('Evaluation start!')
    model.eval()
    if config.task == 'QG':
...
def prepare_real_life_evaluation():
    ...
    batch_of_text = []
    with open(os.path.join(real_dir, "real_input.txt")) as f:
        batch = ""
...
if __name__ == '__main__':
    ...
    # Run Evaluation
    evaluate4(test_loader, model, config.load_ckpt, config, test=True)
