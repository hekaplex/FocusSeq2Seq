...
def preprocess_data(data_dir, split='train', n_process=4, max_len=None):
    ...
    input_df = pd.read_pickle(os.path.join(data_dir, split + ".pkl"))
    # _src_data, _tgt_data = load_data(data_dir, split)
    _src_data = list(input_df.Text.values)
    _tgt_data = list(input_df.Summary.values)
...
def preprocess_data_for_test(data_dir,input_data,word2id, id2word, split='train', n_process=4, max_len=None):
    print(f'Preprocessing {split} dataset...')
    _src_data = input_data
    _tgt_data = input_data
    ...
    # tgt_data = tgt_data[:10]
    with Pool(n_process) as pool:
        data = list(tqdm(pool.imap(partial(preprocess_single_example_real,word2id, id2word,
                                           split=split),
                                   zip(src_data, tgt_data)),
                         total=valid_total_len))
...
def preprocess_single_example_real(word2id, id2word,src_tgt, split='train'):
    src, tgt = src_tgt
